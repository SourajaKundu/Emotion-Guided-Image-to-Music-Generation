...
from tensorflow.keras.models import Model
from keras.layers import Input, Dropout, Dense, Embedding, Add, MultiHeadAttention, LayerNormalization, GlobalAveragePooling1D, Flatten, Dense, Reshape
...
songs = glob('/home/souraja/All_Data_OneAI_Souraja_Final/Image_to_Sequence/train_data/audio/*.mid')
songs.sort()
...
if gpus:
    try:
        # Specify GPU(s) to use, here using the first GPU
        tf.config.experimental.set_visible_devices(gpus[0], 'GPU')

        # Allow growth for memory (useful to prevent TensorFlow from reserving all GPU memory)
...
def custom_loss(y_true, y_pred, weight=0.0001):
    try:
        with tf.device("/GPU:0"):  # Or specify your desired device
            loss_values = y_true
        ...
        filepath = '/home/souraja/All_Data_OneAI_Souraja_Final/Image_to_Sequence/music.keras'
        model = tf.keras.models.load_model(filepath)
        VA_true = model(rep_true)
        VA_pred = model(rep_pred)
        new_loss = tf.reduce_mean(tf.abs(VA_true - VA_pred))
        categorical_crossentropy = tf.reduce_mean(tf.keras.losses.categorical_crossentropy(y_true, y_pred))
        total_loss = 0.0000001 * new_loss + categorical_crossentropy

    except Exception as e:
        print(f"Exception occurred during VA loss calculation: {e}")
...
def create_model_with_2_transformer(vocab_size, max_length, feature_extractor_label="VG16"):
...
def create_model_trans_enc3(vocab_size, max_length, feature_extractor_label="VG16"):
    # Image feature extractor model
    if feature_extractor_label == "VG16":
        inputs1 = Input(shape=(4096,))
    elif feature_extractor_label == "InceptionV3":
...
#def create_model_with_3_enc_dec_transformer(vocab_size, max_length, feature_extractor_label="InceptionV3"):
    # Image feature extractor model
    if feature_extractor_label == "VG16":
        inputs1 = Input(shape=(4096,))
    elif feature_extractor_label == "InceptionV3":
    ...
    # Merging model
    model = Model(inputs=[inputs1, inputs2], outputs=outputs)
    #model.compile(loss=custom_loss, optimizer=Adam(learning_rate=0.00001))
...
def create_model_with_transformer_enc2_dec2(vocab_size, max_length, feature_extractor_label="VG16"):
    # Image feature extractor model
    if feature_extractor_label == "VG16":
        inputs1 = Input(shape=(4096,))
    elif feature_extractor_label == "InceptionV3":
...
#iterate over each midi file
  ...
  for file in songs:
    ...
    jpgFilenamesList = ['EIMG\paper1\train_data\image' + os.path.basename(file).split('.')[0] + '.jpg']
...
#tokenizer = load_featuresPickle('/home/souraja/All_Data_OneAI_Souraja_Final/Image_to_Sequence/tokenizer.pkl')
...
def create_Model_orig(vocabSize, maxLength):
  #image feature extractor model
...
def create_sequences(tokenizer, max_length, midi_list, photo, vocab_size):
  ...
  for midi in midi_list:
    ...
    # create_midi(prediction_list_true, 'ref_midi')
...
def data_generator(midiData, photos, tokenizer, max_length, vocab_size):
  ...
  while True:
    for key, midi_list in midiData.items():
      ...
      # create_midi(prediction_list_true, 'new_midi')
      yield (in_img, in_seq), out_word
...
def create_midi(prediction_output, midiName):
  ...
  midi_stream.write('midi', fp= '/home/souraja/All_Data_OneAI_Souraja_Final/Image_to_Sequence/Generated_MIDI/testMIDI_trans_enc3_dec3/trial/' + midiName + '.mid')
...
# features = extract_Image_features(r"D:\Audio\Models\EIMG\paper1\train_data\image", "InceptionV3")
# #print length of features
# print("Extracted features : %d" % len(features))
# # save to pickle file
# dump(features, open('features_incept.pkl', 'wb'))
train_features = load_featuresPickle('/home/souraja/All_Data_OneAI_Souraja_Final/Image_to_Sequence/features.pkl')
...
#create model
model = create_model_with_3_enc_dec_transformer(vocabSize, maxLength)
...
filepath = '/home/souraja/All_Data_OneAI_Souraja_Final/training_new_model/trans_enc3_dec3.weights.h5'
...
# filepath = r"D:\Audio\Models\EIMG\paper1\trans3encdec.weights.h5"
model.load_weights(filepath)
# # give the path to testImages, here testImages = image
images = glob('/home/souraja/All_Data_OneAI_Souraja_Final/Image_to_Sequence/test_data/test4/*.jpg')
