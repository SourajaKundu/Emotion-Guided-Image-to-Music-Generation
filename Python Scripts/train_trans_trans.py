...
def create_model_with_transformer(vocab_size, max_length, feature_extractor_label="VG16"):
...
def create_model_with_4_transformer_keras_decoder_MIDI(vocab_size, max_length, feature_extractor_label="VG16"):
    # Image feature extractor model
    if feature_extractor_label == "VG16":
        inputs1 = Input(shape=(4096,))
    elif feature_extractor_label == "InceptionV3":
    ...
    decoder_attention_output = MultiHeadAttention(num_heads=4, key_dim=256)(decoder2, decoder2, decoder2)
    decoder_attention_output = LayerNormalization(epsilon=1e-6)(decoder_attention_output)
    decoder_attention_output = Dropout(0.2)(decoder_attention_output)
    
    decoder3 = Dense(256, activation='relu')(decoder_attention_output)
    outputs = Dense(vocab_size, activation='softmax')(decoder3)

    # Merging model
    model = Model(inputs=[inputs1, inputs2], outputs=outputs)
    model.compile(loss='categorical_crossentropy', optimizer=Adam(learning_rate=0.00002))
...
def create_model_with_4_transformer_decoder_MIDI(vocab_size, max_length, feature_extractor_label="VG16"):
    ...
    transdec1 = MultiHeadAttention(num_heads=4, key_dim=256)(decoder1, decoder1, return_attention_scores=True)
...
def create_midi(prediction_output, midiName):
  ...
  midi_stream.write('midi', fp='testMidi_trans/' + midiName + '.mid')
...
train_features = load_featuresPickle('features.pkl')     #VGG
...
# model = create_model_with_4_transformer_decoder_MIDI(vocab_size= vocabSize, max_length=maxLength)
...
checkpoint = ModelCheckpoint(filepath, monitor='loss', verbose=1, save_weights_only=True, save_best_only=False, mode='min')
